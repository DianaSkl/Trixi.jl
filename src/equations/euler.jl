@muladd begin
    ...
    """
    struct EulerEquations1D{RealT<:Real} <: AbstractCompressibleEulerEquations{1, 3}
      gamma::RealT               # ratio of specific heats
      inv_gamma_minus_one::RealT # = inv(gamma - 1); can be used to write slow divisions as fast multiplications
    
      function EulerEquations1D(gamma)
    ...
    """
        initial_condition_constant(x, t, equations::EulerEquations1D)
    ...
    """
        initial_condition_convergence_test(x, t, equations::EulerEquations1D)
    
    A smooth initial condition used for convergence tests in combination with
    [`source_terms_convergence_test`](@ref)
    (and [`BoundaryConditionDirichlet(initial_condition_convergence_test)`](@ref) in non-periodic domains).
    """
    function initial_condition_convergence_test(x, t, equations::EulerEquations1D)
      c = 2
      A = 0.1
      L = 2
      f = 1/L
    ...
    """
        source_terms_convergence_test(u, x, t, equations::bleEulerEquations1D)
    ...
    @inline function source_terms_convergence_test(u, x, t, equations::EulerEquations1D)
      ...
      #                          (γ - 1) * cos((t - x1) * ω) * A * ω) / 2
        ...
        du1 = 0
        du2 = (2*ω*cos((x1-t)*ω)*sin((x1-t)*ω)+35*ω*cos((x1-t)*ω))/150
        du3 = (2*ω*cos((x1-t)*ω)*sin((x1-t)*ω)+35*ω*cos((x1-t)*ω))/150
    ...
    """
        initial_condition_density_pulse(x, t, equations::EulerEquations1D)
    ...
    """
    function initial_condition_density_pulse(x, t, equations::EulerEquations1D)
      rho = 1 + exp(-(x[1]^2 ))/2
      v1 = 1
      rho_v1 = rho * v1
      p = 1
    ...
    """
        initial_condition_density_wave(x, t, equations::EulerEquations1D)
    ...
    - polydeg = 5
    """
    function initial_condition_density_wave(x, t, equations::EulerEquations1D)
      v1 = 0.1
    ...
    """
        initial_condition_weak_blast_wave(x, t, equations::EulerEquations1D)
    ...
    """
    function initial_condition_weak_blast_wave(x, t, equations::EulerEquations1D)
    ...
    """
        initial_condition_blast_wave(x, t, equations::EulerEquations1D)
    ...
    - Sebastian Hennemann, Gregor J. Gassner (2020)
      ...
      [arXiv: 2008.12044](https://arxiv.org/abs/2008.12044)
    """
    function initial_condition_blast_wave(x, t, equations::EulerEquations1D)
    ...
    """
        initial_condition_sedov_blast_wave(x, t, equations::EulerEquations1D)
    
    The Sedov blast wave setup based on Flash
    - http://flash.uchicago.edu/site/flashcode/user_support/flash_ug_devel/node184.html#SECTION010114000000000000000
    """
    function initial_condition_sedov_blast_wave(x, t, equations::EulerEquations1D)
      # Set up polar coordinates
      inicenter = SVector(0.0)
      x_norm = x[1] - inicenter[1]
      r = abs(x_norm)
    ...
    """
        initial_condition_eoc_test_coupled_euler_gravity(x, t, equations::EulerEquations1D)
    ...
    !!! note
        ...
        present there.
    """
    function initial_condition_eoc_test_coupled_euler_gravity(x, t, equations::EulerEquations1D)
    ...
    """
        flux_shima_etal(u_ll, u_rr, orientation, equations::EulerEquations1D)
    ...
    """
    @inline function flux_shima_etal(u_ll, u_rr, orientation::Integer, equations::EulerEquations1D)
    ...
    """
        flux_kennedy_gruber(u_ll, u_rr, orientation, equations::EulerEquations1D)
    ...
    """
    @inline function flux_kennedy_gruber(u_ll, u_rr, orientation::Integer, equations::EulerEquations1D)
      # Unpack left and right state
      rho_e_ll = last(u_ll)
      rho_e_rr = last(u_rr)
      rho_ll, v1_ll, p_ll = cons2prim(u_ll, equations)
    ...
    """
        flux_chandrashekar(u_ll, u_rr, orientation, equations::EulerEquations1D)
    ...
    """
    @inline function flux_chandrashekar(u_ll, u_rr, orientation::Integer, equations::EulerEquations1D)
      # Unpack left and right state
      rho_ll, v1_ll, p_ll = cons2prim(u_ll, equations)
      rho_rr, v1_rr, p_rr = cons2prim(u_rr, equations)
    ...
    """
        flux_ranocha(u_ll, u_rr, orientation, equations::EulerEquations1D)
    ...
    """
    @inline function flux_ranocha(u_ll, u_rr, orientation::Integer, equations::EulerEquations1D)
      # Unpack left and right state
      rho_ll, v1_ll, p_ll = cons2prim(u_ll, equations)
      rho_rr, v1_rr, p_rr = cons2prim(u_rr, equations)
    ...
    @inline function min_max_speed_naive(u_ll, u_rr, orientation::Integer, equations::EulerEquations1D)
      rho_ll, v1_ll, p_ll = cons2prim(u_ll, equations)
      rho_rr, v1_rr, p_rr = cons2prim(u_rr, equations)
    ...
    """
        flux_hllc(u_ll, u_rr, orientation, equations::EulerEquations1D)
    ...
    """
    function flux_hllc(u_ll, u_rr, orientation::Integer, equations::EulerEquations1D)
      # Calculate primitive variables and speed of sound
      rho_ll, rho_v1_ll, rho_e_ll = u_ll
      rho_rr, rho_v1_rr, rho_e_rr = u_rr
    ...
    @inline function prim2cons(prim, equations::EulerEquations1D)
      rho, v1, p = prim
      rho_v1 = rho * v1
    ...
    @inline function pressure(u, equations::EulerEquations1D)
     rho, rho_v1, rho_e = u
    ...
    @inline function entropy_thermodynamic(cons, equations::EulerEquations1D)
      # Pressure
    ...
    @inline function entropy_math(cons, equations::EulerEquations1D)
    ...
    @inline entropy(cons, equations::EulerEquations1D) = entropy_math(cons, equations)
    
    
    # Calculate total energy for a conservative state `cons`
    @inline energy_total(cons, ::EulerEquations1D) = cons[3]
    
    
    # Calculate kinetic energy for a conservative state `cons`
    @inline function energy_kinetic(cons, equations::EulerEquations1D)
      return 0.5 * (cons[2]^2)/cons[1]
    end
    
    
    # Calculate internal energy for a conservative state `cons`
    @inline function energy_internal(cons, equations::EulerEquations1D)
